...
sudo apt-get install -qq libatlas3-base
...
sudo add-apt-repository "deb [trusted=yes] http://llvm.org/apt/precise/ llvm-toolchain-precise main"
sudo add-apt-repository "deb [trusted=yes] http://llvm.org/apt/precise/ llvm-toolchain-precise-$LLVMVER main"
sudo apt-get update
